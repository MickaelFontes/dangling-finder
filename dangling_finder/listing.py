    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            if resp.status == 200:
                ...
                commit_hashes = [
                    link["href"].split("/")[-1].split("..")[0] for link in push_link
                ]
                if len(commit_hashes) > 0:
                    await save_dangling_commits_list(
                        owner, repo, commit_hashes, pull_nb
                    )
...
def main(owner, repo, pr_max):
    loop = asyncio.get_event_loop()
    loop.run_until_complete(process(owner, repo, pr_max))
